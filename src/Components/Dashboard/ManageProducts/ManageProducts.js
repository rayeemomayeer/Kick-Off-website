    ...
    <div className="max-w-2xl mx-auto pb-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      <div className="mt-6 grid grid-cols-1 gap-y-16 gap-x-6 sm:grid-cols-2 lg:grid-cols-2 xl:gap-x-8">
		...
		<div class="w-full md:w-1/3 h-full bg-white grid place-items-center">
			<img src={tour?.image} class="rounded-xl h-full object-cover w-full" />
      <ToastContainer
position="top-right"
autoClose={5000}
hideProgressBar
...
/>
			...
			<div class="w-full md:w-2/3 bg-white flex flex-col space-y-2 p-3">
				<div class="flex justify-between item-center">
					<p class="text-gray-500 font-medium hidden md:block">{tour?.reviews} reviews</p>
				...
				<h3 class="font-black text-gray-800 md:text-xl text-xl">{tour?.name}</h3>
				<p class="text-gray-500 text-base">{tour?.description?.slice(0, 150)}...</p>
          ...
          <p class="text-lg font-black text-gray-800">
                  ...
                  <div>
                    <button onClick={()=> handleDelete(tour?._id)} className="px-2 py-1 text-xs font-semibold text-white uppercase transition-colors duration-200 transform bg-indigo-700 rounded hover:bg-indigo-700 dark:hover:bg-red-600 focus:bg-indigo-700 dark:focus:bg-gray-600 focus:outline-none mr-2">edit</button>
