...
const ProductDescription = () => {
  ...
  const [buttonText, setButtonText] = useState("Add to The Cart");
  useEffect(() => {
    fetch('/products.json')
      .then(res=>res.json())
      .then(data=>{
  ...
  const onSubmit = () => {
    ...
    .then(res=>{
      if(res.data.insertedId){
        toast.success('Added in to the Cart', {
position: "top-right",
autoClose: 5000,
...
<Link to="/home"><button className="text-green-700 text-2xl bg-green-200 rounded px-2 ml-2"><svg xmlns="http://www.w3.org/2000/svg" class="h-9 w-9 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  <div className="lg:row-span-3">
    ...
    <button disabled={disable} id="book-hotel-btn" onClick={onSubmit} className="mt-10 w-full bg-indigo-600 border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">{buttonText}</button>
  </div>
</div>
      </div>
